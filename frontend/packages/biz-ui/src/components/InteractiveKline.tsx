...
export default function InteractiveKline(props: {
  ...
  const dense = isCompact || isMini
  const chartHeight = isMini ? 168 : isCompact ? 220 : 380
  const macdHeight = isCompact ? 86 : 150
  ...
  const load = async () => {
    ...
    try {
      const query = (days: number) =>
        props.endpointBuilder
          ? props.endpointBuilder({ symbol: props.symbol, market: props.market, days, interval })
  ...
  return (
    <div className={`card ${dense ? 'p-2.5' : 'p-4 md:p-5'}`}>
      ...
      <div className="relative">
        {showSkeleton ? (
          <div className="w-full rounded-xl overflow-hidden border border-border/50 p-3 animate-pulse" style={{ height: chartHeight }}>
            <div className="h-full w-full rounded-lg bg-accent/20" />
          </div>
        ) : (
          <div ref={containerRef} className="w-full rounded-xl overflow-hidden border border-border/50" style={{ height: chartHeight }} />
        )}
      ...
      {!isMini && <div className={`${dense ? 'mt-2' : 'mt-3'} grid grid-cols-1 gap-3`}>
        <div>
          <div className="text-[11px] text-muted-foreground mb-1">动能指标（MACD{showRsi ? ' + RSI强弱线' : ''}）</div>
          {!dense && <div className="text-[11px] text-muted-foreground mb-2 rounded-lg bg-accent/15 border border-border/40 px-2.5 py-1.5">
            MACD 用来看趋势动能和拐点；RSI 用来看是否偏热/偏弱（一般 70 以上偏热，30 以下偏弱）。
          </div>}
          {showSkeleton ? (
            <div className="w-full rounded-xl overflow-hidden border border-border/50 animate-pulse" style={{ height: macdHeight }}>
              <div className="h-full w-full bg-accent/20" />
            </div>
          ) : (
            <div ref={macdRef} className="w-full rounded-xl overflow-hidden border border-border/50" style={{ height: macdHeight }} />
          )}
        </div>
      </div>}
    </div>
