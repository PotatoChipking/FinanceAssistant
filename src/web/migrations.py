...
def _m119_stock_screener_tables(conn: Connection) -> None:
    """Create formula screener persistence tables."""

    conn.execute(
        text(
            """
CREATE TABLE IF NOT EXISTS stock_screener_formulas (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  name TEXT NOT NULL,
  description TEXT DEFAULT '',
...
CREATE TABLE IF NOT EXISTS stock_screener_runs (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  formula_id INTEGER REFERENCES stock_screener_formulas(id) ON DELETE SET NULL,
...
CREATE TABLE IF NOT EXISTS stock_screener_results (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  run_id INTEGER NOT NULL REFERENCES stock_screener_runs(id) ON DELETE CASCADE,
...
"""
    ...
    _create_index_if_missing(
        conn,
        "ix_screener_formula_enabled",
        "CREATE INDEX ix_screener_formula_enabled ON stock_screener_formulas(enabled, updated_at)",
    )
    _create_index_if_missing(
        conn,
        "ix_screener_run_created",
        "CREATE INDEX ix_screener_run_created ON stock_screener_runs(created_at)",
    )
    _create_index_if_missing(
        conn,
        "ix_screener_run_status",
        "CREATE INDEX ix_screener_run_status ON stock_screener_runs(status, created_at)",
    )
    _create_index_if_missing(
        conn,
        "ix_screener_result_run",
        "CREATE INDEX ix_screener_result_run ON stock_screener_results(run_id, matched)",
    )
    _create_index_if_missing(
        conn,
        "ix_screener_result_symbol",
        "CREATE INDEX ix_screener_result_symbol ON stock_screener_results(market, symbol)",
    )


def _m120_screener_task_fields(conn: Connection) -> None:
    """Add task/progress columns used by managed screener runs."""

    _add_column_if_missing(
        conn,
        "stock_screener_runs",
        "task_id",
        "ALTER TABLE stock_screener_runs ADD COLUMN task_id TEXT DEFAULT ''",
    )
    _add_column_if_missing(
        conn,
        "stock_screener_runs",
        "progress_current",
        "ALTER TABLE stock_screener_runs ADD COLUMN progress_current INTEGER DEFAULT 0",
    )
    _add_column_if_missing(
        conn,
        "stock_screener_runs",
        "progress_total",
        "ALTER TABLE stock_screener_runs ADD COLUMN progress_total INTEGER DEFAULT 0",
    )
    _add_column_if_missing(
        conn,
        "stock_screener_runs",
        "started_at",
        "ALTER TABLE stock_screener_runs ADD COLUMN started_at DATETIME",
    )
    _create_index_if_missing(
        conn,
        "ix_screener_run_task",
        "CREATE INDEX ix_screener_run_task ON stock_screener_runs(task_id)",
    )
