...
class WatchedBoard(Base):
    """User-selected market board for the event calendar."""

    __tablename__ = "watched_boards"
    __table_args__ = (
        UniqueConstraint("market", "board_code", name="uq_watched_board_market_code"),
        Index("ix_watched_boards_enabled", "enabled", "sort_order"),
    )
...
class BoardKlineCache(Base):
    ...
    __tablename__ = "board_kline_cache"
    __table_args__ = (
        UniqueConstraint(
            "market",
            "board_code",
            "date",
            name="uq_board_kline_market_code_date",
        ),
        Index("ix_board_kline_lookup", "market", "board_code", "date"),
    )
...
class StockScreenerFormula(Base):
    ...
    __tablename__ = "stock_screener_formulas"
    __table_args__ = (
        Index("ix_screener_formula_enabled", "enabled", "updated_at"),
    )
    ...
    universe_config = Column(JSON, default={})
    enabled = Column(Boolean, default=True)
    created_at = Column(DateTime, server_default=func.now())
    updated_at = Column(DateTime, server_default=func.now(), onupdate=func.now())
    runs = relationship("StockScreenerRun", back_populates="formula")


class StockScreenerRun(Base):
    """Execution history for a stock screener formula."""

    __tablename__ = "stock_screener_runs"
    __table_args__ = (
        Index("ix_screener_run_created", "created_at"),
        Index("ix_screener_run_status", "status", "created_at"),
    )

    id = Column(Integer, primary_key=True, autoincrement=True)
    formula_id = Column(
        Integer,
        ForeignKey("stock_screener_formulas.id", ondelete="SET NULL"),
        nullable=True,
    )
    ...
    formula = relationship("StockScreenerFormula", back_populates="runs")
...
class StockScreenerResult(Base):
    """Matched stock row for a screener run."""

    __tablename__ = "stock_screener_results"
    __table_args__ = (
        Index("ix_screener_result_run", "run_id", "matched"),
        Index("ix_screener_result_symbol", "market", "symbol"),
    )

    id = Column(Integer, primary_key=True, autoincrement=True)
    run_id = Column(
        Integer,
        ForeignKey("stock_screener_runs.id", ondelete="CASCADE"),
        nullable=False,
    )
    symbol = Column(String, nullable=False)
    market = Column(String, nullable=False, default="CN")
    name = Column(String, default="")
    board_code = Column(String, default="")
